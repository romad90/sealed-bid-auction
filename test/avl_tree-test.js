...
describe('AVLTree', () => {
  ...
  it('getBalance(node), should return -1 the difference in the current BST between the height left < right ', done => {
	...
	it('isBSTBalanced(), should keep the BST balanced', done => {
    const bst = new AVLTree()
    bst.insert(2)
    bst.insert(1, 'alfa')
    bst.insert(3, 'bravo')
	...
	it('isBSTBalanced(), should perform a LL rotation(rotate anticlockwise) to keep the BST balanced despite the following number not suitable for a balanced BST', done => {
    const bst = new AVLTree()
    bst.insert(1)
    bst.insert(2, 'alfa')
    bst.insert(3, 'bravo')
    expect(bst.root.key).to.be.equal(2)
    expect(bst.root.left.key).to.be.equal(1)
    expect(bst.root.right.key).to.be.equal(3)
    expect(bst.isBSTBalanced()).to.be.equal(true)
    done()
	})
	it('isBSTBalanced(), should perform a RR rotation(rotate clockwise) to keep the BST balanced despite the following number not suitable for a balanced BST', done => {
    const bst = new AVLTree()
    bst.insert(3)
    bst.insert(2, 'alfa')
    bst.insert(1, 'bravo')
	...
	it('isBSTBalanced(), should perform a LR rotation (LL + RR) to keep the BST balanced despite the following number not suitable for a balanced BST', done => {
    const bst = new AVLTree()
    bst.insert(3)
    bst.insert(1, 'bravo')
    bst.insert(2, 'alfa')
	...
	it('isBSTBalanced(), should perform a RL rotation(RR + LL) to keep the BST balanced despite the following number not suitable for a balanced BST', done => {
    const bst = new AVLTree()
    bst.insert(1)
    bst.insert(3, 'bravo')
    bst.insert(2, 'alfa')
