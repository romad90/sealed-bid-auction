...
class Node {
   ...
   */
  constructor(key, bidderName='*reservePrice') {
  ...
  addLeftNode(node) {
    if(!node || node.constructor !== Node) throw new Error('must be a Node object!')
    if(node.key > this.key) throw new Error(`New node value must be lesser than the current one : ${node.key} > ${this.key}.`)
    this.left = node
  }
  addRightNode(node) {
    if(!node || node.constructor !== Node) throw new Error('must be a Node object!')
    if(node.key < this.key) throw new Error(`New node value must be greater than the current one : ${node.key} < ${this.key}.`)
    this.right = node
  }
}
